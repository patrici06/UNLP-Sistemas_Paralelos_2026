...
# ============================================================
# Script de ejecución LOCAL para matrices-pthreads.c
# ============================================================
...
#   - Threads: T{2,4,8} (PARÁMETRO al programa)
#   - Optimización: O{1,2,3}
#   - Ejecución: local vs cluster
#
# IMPORTANTE: Los threads se pasan por PARÁMETRO
# Uso: ./matrices-pthreads N T
# ============================================================
...
echo "======================================"
echo "SCRIPT DE EJECUCIÓN LOCAL - PTHREADS"
echo "======================================"
echo "Directorio de salidas: $SALIDAS_DIR/$TIPO_EXEC"
echo "Threads: ${thread_counts[@]} (por PARÁMETRO)"
echo ""
...
process_output() {
...

	{
		...
		echo "Threads: $thread_count (parámetro)"
...
for n in "${matrix_sizes[@]}"; do
	echo "=================================================="
	echo "Procesando: N=$n (PTHREADS)"
	echo "=================================================="

	for t in "${thread_counts[@]}"; do
		for o in "${optimization_levels[@]}"; do
			# Estructura: salidas/pthreads/N512/T2/O1/{outputs,reports}
			...
			# Ejecutar con threads como PARÁMETRO
			timestamp=$(date +%s%N)
			output=$(./matrices-pthreads-O${o} $n $t 2>&1)
			exit_code=$?

			# Guardar salida
			output_file="$OUTPUT_DIR/pthreads_${timestamp}.txt"
			echo "$output" > "$output_file"

			# Crear reporte
			report_file="$REPORT_DIR/pthreads_${timestamp}.txt"
