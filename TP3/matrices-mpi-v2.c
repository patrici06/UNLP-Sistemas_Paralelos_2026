...
int main(int argc, char *argv[]) {
    ...
    *r = NULL;
    ...
    // SCATTER: repartir A y B
    MPI_Scatter(a, stripSize * n, MPI_DOUBLE, localA, stripSize * n,
                MPI_DOUBLE, COORDINADOR, MPI_COMM_WORLD);
    MPI_Scatter(b, stripSize * n, MPI_DOUBLE, localB, stripSize * n,
                MPI_DOUBLE, COORDINADOR, MPI_COMM_WORLD);

    tick[1] = MPI_Wtime();

    // ETAPA 0: Estadisticas (cada proceso sobre su franja local)
    double localMinA = localA[0], localMaxA = localA[0], localSumA = 0.0;
    double localMinB = localB[0], localMaxB = localB[0], localSumB = 0.0;
    ...
    // ETAPA 1: D = B x B^T
    //
    //   Todos necesitan B completa. El coordinador usa su propia b,
    //   los demas hacen Allgather localB -> fullB.
    //
    //   Cada proceso escribe su franja de D directamente en formato
    //   compacto: partialD[localRow + col * stripSize]
    //   (column-major con stride = stripSize en vez de n).
    //
    //   Esto evita el buffer fullD n*n intermedio y el empaque
    //   posterior. blkmulRowColCol se parametriza con ldc (stride
    //   de salida) para escribir en el formato compacto.
    //
    //   Luego MPI_Gather de strips compactos al coordinador, que
    //   reordena a column-major clasico (stride n) en d.
    // ==================================================================

    double *fullB = NULL;
    // reservamos espacio para la B completa en cada proceso (menos si somo coordinador que ya la conoce)
    if (rank != COORDINADOR)
        fullB = (double *)malloc(sizeof(double) * n * n);

    tick[4] = MPI_Wtime();
    
    // Broadcast de B completa desde coordinador esto para evitar el Allgather que es mas costoso, el coordinador ya tiene B completa en b, los demas la reciben en fullB
    if (rank == COORDINADOR) {
        MPI_Bcast(b, n * n, MPI_DOUBLE, COORDINADOR, MPI_COMM_WORLD);
    } else {
        MPI_Bcast(fullB, n * n, MPI_DOUBLE, COORDINADOR, MPI_COMM_WORLD);
    }
    
    tick[5] = MPI_Wtime();
    
    double *Bcomp = (rank == COORDINADOR) ? b : fullB;

    // partialD: franja de D
    double *partialD = (double *)calloc((size_t)stripSize * n, sizeof(double));

    for (int i = stripStart; i < stripStart + stripSize; i += BS) {
        for (int j = 0; j < n; j += BS) {
            for (int k = 0; k < n; k += BS) {
                blkmulRowColRow(&Bcomp[i * n + k], &Bcomp[j * n + k],
    ...
    tick[6] = MPI_Wtime();
     ...
     if (fullB) free(fullB);
       ...
       // Allgather de D: cada proceso envía su strip row-major
       double *fullD = (double *)malloc(sizeof(double) * n * n);
      ...
      // Etapa 2: R = A x D (row-major)
    double *localR = (double *)calloc((size_t)stripSize * n, sizeof(double));
    ...
    tick[9] = MPI_Wtime();
    // ETAPA 3: R = constante * R (element-wise, cada uno su franja)
    tick[10] = MPI_Wtime();
    for (int idx = 0; idx < stripSize * n; idx++) {
        localR[idx] *= constante;
    }
    tick[11] = MPI_Wtime();
    // Gather final: reunir R en el coordinador
    // localR ya es row-major compacto -> Gather directo
    tick[12] = MPI_Wtime();
    //MPI_Allgather(partialD, stripSize * n, MPI_DOUBLE,
    //fullD, stripSize * n, MPI_DOUBLE, MPI_COMM_WORLD);
