...
@graph("overhead", "Overhead de comunicación (barras agrupadas por N)")
def generate(df, output_dir):
    ...
    for i, cfg in enumerate(setups):
        ...
        for n in ns:
            ...
            if not row.empty:
                values.append(row['comm_pct'].values[0])
            else:
                values.append(0)
    ...
    ax.set_ylabel('Overhead de comunicación (%)')
    ax.set_title('Overhead de comunicación por configuración')
    ax.set_xticks(np.arange(n_groups))
    ax.set_xticklabels([str(n) for n in ns])
    ax.legend(fontsize=8)

    style.save_plot(fig, f"{output_dir}/overhead")
