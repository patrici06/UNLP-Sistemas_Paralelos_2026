...
//Funciones heredadas de TP2, veremos si tienen sentido despues
double dwalltime();
...
int main(int argc, char* argv[]){
    ...
    */
	...
	// D en column-major y R en row-major
    ...
    // Paralelizamos con OpenMP dentro de cada proceso: cada proceso calcula su franja de filas de D
    // Mantenemos un grano gueso para maximizar cache hits, cada iteracion del for externo calcula un bloque de filas de D (stripSize filas)
    ...
    // D está distribuido en column-major (desde Etapa 1).
    ...
    // Paralelizar con OpenMP dentro de cada proceso: cada proceso calcula su franja de filas de R
    // Paralelizamos en esta etapa para mantener una granularidad gruesa, esto debe mejorar significativamente la taza de cache hits.
    #pragma omp parallel for schedule(static)
    for (i = stripStart; i < stripStart + stripSize; i += BS) {
        int in = i * n;
        for (j = 0; j < n; j += BS) {
